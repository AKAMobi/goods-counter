...
def load_data(path_positive,path_negative):
	...
	list_1 = range(num_positive)
	random.shuffle(list_1)
	list_0 = range(num_negative)
        random.shuffle(list_0)
	for i in range(num_positive):
		img = Image.open(path_positive+'/'+imgs_positive[list_1[i]])
		arr = np.asarray(img,dtype="float32")
		if i < num_positive/5:
			X_valid[i,:,:,:] = [arr[:,:,0],arr[:,:,1],arr[:,:,2]]
                	Y_valid[i] = 1
		elif i < 2*(num_positive/5):
			X_test[i-num_positive/5,:,:,:] = [arr[:,:,0],arr[:,:,1],arr[:,:,2]]
                        Y_test[i-num_positive/5] = 1
		else:
			X_train[i-2*(num_positive/5),:,:,:] = [arr[:,:,0],arr[:,:,1],arr[:,:,2]]
			Y_train[i-2*(num_positive/5)] = 1
	for i in range(num_negative):
                img = Image.open(path_negative+'/'+imgs_negative[list_0[i]])
                arr = np.asarray(img,dtype="float32")
                if i < num_negative/5:
                        X_valid[num_positive/5+i,:,:,:] = [arr[:,:,0],arr[:,:,1],arr[:,:,2]]
                        Y_valid[num_positive/5+i] = 0
                elif i < 2*(num_negative/5):
                        X_test[num_positive/5+i-num_negative/5,:,:,:] = [arr[:,:,0],arr[:,:,1],arr[:,:,2]]
                        Y_test[num_positive/5+i-num_negative/5] = 0
                else:
                        X_train[num_positive-2*(num_positive/5)+i-2*(num_negative/5),:,:,:] = [arr[:,:,0],arr[:,:,1],arr[:,:,2]]
                        Y_train[num_positive-2*(num_positive/5)+i-2*(num_negative/5)] = 0
